...
class InterfaceCompatibilityDetector():
    def __init__(self):
        self.init()
    ...
    def detect_interface_compatibility(self, previous_instruction, current_instruction, tainted_record, transaction_index, contract_dict, source_map):
        ...
        if current_instruction["op"] == "CALL" and convert_stack_value_to_int(current_instruction["stack"][-5]) > 0:
            ...
            if source_map and source_map.get_buggy_line(current_instruction["pc"]):
                callerfunction=source_map.get_buggy_line(current_instruction["pc"])
                spotSite=callerfunction.find(".")
                # todo spotSite位置的判断
                kuohaoSite=callerfunction.find("(",spotSite)
                calledfunction=callerfunction[spotSite+1:kuohaoSite]
                if calledfunction not in contract_function_name:
